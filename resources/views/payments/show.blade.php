...
@section('title', ucfirst($payment->direction === 'in' ? 'Customer' : 'Supplier') . ' payment — ' . $payment->payment_date->format('Y-m-d'))
    ...
    <div class="max-w-3xl space-y-6">
        ...
        <div class="rounded-lg border border-gray-200 bg-white">
            ...
            @forelse ($payment->allocations as $allocation)
                <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2 text-sm last:border-0">
            ...
            @if ($unallocated > 0.001)
                <div class="flex items-center justify-between px-4 py-2 text-sm text-amber-600">
        ...
        @if ($payment->notes)
            <div class="rounded-lg border border-gray-200 bg-white p-4 text-sm text-gray-600">{{ $payment->notes }}</div>
        @endif
